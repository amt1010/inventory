...
<tbody>
 ...
 <?php
            ...
            $products = DB::table('admn_qery')->get();
            ?>
...
<?php
$cid = $product->user_id;
  $vip =User::when($cid, function ($query,$cid ) {
...
<td><form action="{{ url('updtquery/'.$product->id) }}" method="POST">
    @method('PUT')
    @csrf
...
<td>
<form class="forms-sample" method="POST" action="{{url('deletequery')}}" enctype="multipart/form-data">
                  @csrf
                  @method('post')
